...
install_update() {
  ...
  if [ -d "$update_content_dir/backend/src" ]; then
    cp -r "$update_content_dir/backend/src"/* backend/src/
    log "已更新后端代码"
  fi
  
  if [ -d "$update_content_dir/frontend/src" ]; then
    cp -r "$update_content_dir/frontend/src"/* frontend/src/
    log "已更新前端代码"
  fi
  ...
  if [ -d "$update_content_dir/migrations" ]; then
    ...
    for migration in "$update_content_dir/migrations"/*.sql; do
      if [ -f "$migration" ]; then
        mysql -u root -p'JhChat@2026Secure!' jhchat < "$migration"
...
acknowledge_update() {
  ...
  curl -s -X POST "${ADMIN_URL}/api/admin/updates/acknowledge" \
    ...
    -d "{\"version\":\"$version\",\"status\":\"$status\",\"serverName\":\"$SERVER_NAME\"}" > /dev/null || true
  success "确认发送完成"
}
