...
CREATE TABLE IF NOT EXISTS `user_level_config` (
  ...
  `max_daily_chat_exp` INT NOT NULL DEFAULT 500,
  `chat_exp_per_minute` INT NOT NULL DEFAULT 1,
  `can_be_admin` TINYINT(1) NOT NULL DEFAULT 0,
  `min_register_days` INT NOT NULL DEFAULT 0,
  `min_total_exp` BIGINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `chat_exp_logs` (
  ...
  `chat_minutes` INT NOT NULL,
  `is_daily_limit` TINYINT(1) NOT NULL DEFAULT 0,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `admin_applications` (
  ...
  `applied_grade` TINYINT UNSIGNED NOT NULL,
  `reason` TEXT,
...
SET @preparedStatement = (SELECT IF(
  ...
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN `', @columnname, '` INT UNSIGNED NOT NULL DEFAULT 0 AFTER `practice_exp_total`')
));
...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE (table_name = @tablename) AND (table_schema = @dbname) AND (column_name = @columnname)) > 0,
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN `', @columnname, '` INT UNSIGNED NOT NULL DEFAULT 0 AFTER `chat_minutes_today`')
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;
...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE (table_name = @tablename) AND (table_schema = @dbname) AND (column_name = @columnname)) > 0,
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN `', @columnname, '` DATETIME DEFAULT NULL AFTER `chat_minutes_total`')
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;
